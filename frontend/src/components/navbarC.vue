...
<template>
  <nav class="bg-navbarbg py-6 px-2 text-sm font-semibold uppercase">
    <div class="flex items-center justify-between max-lg:block">
      ...
      <button
        class="navbar-toggler items-center float-left absolute top-6 left-44 hidden max-lg:block"
        type="button"
      ...
      >
        <svg
          class="w-8 h-8 text-gray-800 outline-borderC focus:border-borderC"
          aria-hidden="true"
      ...
      <div
        v-if="isOpenMenu"
        class="navbar-collapse flex items-center font-sans font-light max-lg:hidden"
        id="navbarColor04"
      >
      ...
      <div class="relative max-w-xl float-right" v-if="auth.isLoggedIn">
        <button
          class="flex items-center max-w-32 px-3 py-2 bg-primary rounded-md hover:bg-primaryH transition-colors"
          @click="openDropDownUser"
        >
          <p
            class="text-secondary text-md font-sans font-semibold uppercase hover:text-secondaryH transition-colors"
          >
            {{ user?.name }}
          </p>
          <svg
            v-if="!isOpenDropDownUser"
            class="w-6 h-6 text-secondary dark:text-white"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
      ...
      <div
        ...
        :class="[
          'navbar-collapse flex justify-between items-center font-sans font-light max-lg:flex',
