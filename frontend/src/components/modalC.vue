...
const users = computed(() => { return authStore.users })
const is_admin = computed(() => { return authStore.user?.roles?.[0]?.name === 'admin' ? true : false })
...
const date = computed(() => { return new Date().toISOString().split('T')[0] })
