...
<template>
  <div v-if="dash">
    ...
    <TabC>
      <template #dashboard>
        ...
        <div
          class="flex flex-row bg-navbarbg px-1 py-1 rounded-sm shadow-sm w-full max-w-8xl mr-auto max-sm:flex-col"
          v-for="usuario in dash.usuarios.data"
          :key="usuario.id"
        >
          <div class="flex w-full border border-faded rounded-sm">
            ...
            <p class="w-full p-2 font-sans max-w-md text-sm text-center mx-auto">
              {{ usuario.id }}
            </p>
          </div>
          <div class="flex w-full border border-faded rounded-sm">
      ...
      <template #roles>
        <div class="flex gap-4 justify-evenly max-sm:flex-col">
          ...
          <div>
            ...
            <div
              class="flex flex-row bg-navbarbg px-1 py-1 rounded-sm shadow-sm w-full max-w-md mr-auto max-sm:flex-col"
              v-for="rol in dash.roles.data"
              :key="rol.id"
            >
              <div class="flex w-full border border-faded rounded-sm">
                <p
          ...
          <div>
            ...
            <div
              class="flex flex-row bg-navbarbg px-1 py-1 rounded-sm shadow-sm w-full max-w-lg mr-auto max-sm:flex-col"
              v-for="permiso in dash.permisos.data"
              :key="permiso.id"
            >
              <div class="flex w-full border border-faded rounded-sm">
                <p
      ...
      <template #tasks>
        ...
        <h1 class="my-2 font-semibold font-sans text-2xl">
          Cantidad de tareas creadas según usuario
        </h1>
        <div
          class="flex flex-row bg-navbarbg px-1 py-1 rounded-sm shadow-sm w-full max-w-2xl mr-auto max-sm:flex-col"
          v-for="tarea in dash.tareas.por_usuario.cantidad"
          :key="tarea.id"
        >
          <div class="flex w-full border border-faded rounded-sm">
            <p
        ...
        <!-- tareas por estado -->
        <h1 class="my-2 font-semibold font-sans text-2xl">
          Cantidad de tareas según estado y usuario
        </h1>
        <div
          class="flex flex-row bg-navbarbg px-1 py-1 rounded-sm shadow-sm w-full max-w-8xl mr-auto max-sm:flex-col"
          v-for="tarea in dash.tareas.por_estado"
          :key="tarea.id"
        >
          <!-- status in progress -->
          <div v-if="tarea.status === 'in_progress'" class="flex w-full max-w-8xl max-sm:flex-col">
            ...
            <div class="flex w-full border border-faded rounded-sm">
              ...
              <p
                class="w-full p-2 font-sans max-w-md text-sm text-center uppercase text-warning font-semibold mx-auto"
              >
                {{ tarea.status }}
              </p>
            </div>
          ...
          <div v-if="tarea.status === 'pending'" class="flex w-full max-w-8xl max-sm:flex-col">
            ...
            <div class="flex w-full border border-faded rounded-sm">
              ...
              <p
                class="w-full p-2 font-sans uppercase max-w-md text-sm text-center text-danger font-semibold mx-auto"
              >
                {{ tarea.status }}
              </p>
            </div>
          ...
          <div v-if="tarea.status === 'completed'" class="flex w-full max-w-8xl max-sm:flex-col">
            ...
            <div class="flex w-full border border-faded rounded-sm">
              ...
              <p
                class="w-full p-2 font-sans uppercase max-w-md text-sm text-center text-success font-semibold mx-auto"
              >
                {{ tarea.status }}
              </p>
            </div>
  ...
  <div v-if="dash" class="bg-navbarbg px-3 py-3"></div>
</template>
