...
<template>
  <form v-if="taskStore.task && authStore.user" class="max-w-xl mx-auto mt-6 bg-secondary shadow-sm px-3 py-6" @submit.prevent="update">
    <fieldset>
      <h5
    ...
    <div class="relative z-0 w-full mb-5 group">
      ...
      <textarea id="description" v-model="taskStore.task.description" rows="4" class="bg-inputbg text-heading text-sm rounded-sm focus:outline-none block w-full px-3 py-2.5 shadow-xs placeholder:text-body" placeholder="Write your thoughts here..."></textarea>
    </div>
    <div class="relative z-0 w-full mb-5 group">
      <label for="floating_due_date"
        class="lock mb-2.5 text-lg font-sans font-medium text-heading"
      >
        Fecha vencimiento
      </label>
    ...
    <div class="flex w-full">
      <div class="relative z-0 w-full mb-5 group">
        <label for="status"
          class="block mb-2.5 text-lg font-sans font-medium text-heading">
          Estado
        </label>
        <select id="status"
