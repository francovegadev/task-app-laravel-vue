...
watch(internalForm, (val) => emit('update:modelValue', val), { deep: true })
  ...
  <form v-if="internalForm" class="max-w-md mx-auto" @submit.prevent="handleSubmit">
    ...
    <div class="relative z-0 w-full mb-5 group">
      <input
        ...
        required
      />
      <p class="text-danger text-sm capitalize" v-for="(error, idx) in taskStore.errors" :key="idx">
        {{ error }}
      </p>
      <label
        for="floating_title"
