...
<template>
    <div v-if="!taskStore.isLoading && taskStore.tasks" class="w-full max-w-2xl mx-auto mt-6">
        ...
        <div class="w-full mx-auto p-6 bg-navbarbg rounded-sm shadow-md">
            ...
            <div class="flow-root">
                <ul role="list" class="">
                    <li class="pb-4 sm:pb-4 bg-secondary px-3 py-3 rounded-sm shadow-sm my-2"
                        v-for="task in filteredData" :key="task.id">
                        <div class="flex items-center gap-2">
                            <div class="flex-1 min-w-0">
                                ...
                                <p :class="['flex text-sm px-3 py-1 bg-secondary capitalize shadow-sm mt-3 max-w-[120px] rounded-xl text-center font-semibold truncate',
    ...
    <ModalC :show="isOpen" @close="close" title="Crear Tarea">
        <template #body>
            <form @submit.prevent="crear(form)">
