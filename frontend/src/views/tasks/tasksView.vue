...
import FilteredC from '@/components/filteredC.vue'
...
const crear = async (payload: TasksFormInterface) => {
  ...
  if (!taskStore.isLoading) close()
}

const filteredData = computed(() => {
  const collection = taskStore.tasksCollection
...
const deleteRegister = async (id: number | null) => {
  if (id !== null) {
    ...
    if (!taskStore.isLoading) closeModal()
  }
}
...
<template>
  <div v-if="!taskStore.isLoading && taskStore.tasksCollection" class="w-full max-w-2xl mx-auto mt-6">
    ...
    <div class="w-full mx-auto p-6 bg-navbarbg rounded-sm shadow-md">
      <!-- select status tasks -->
      <FilteredC v-model:search-value="searchValue" v-model:selected-status="selectedStatus" />
  ...
  <ModalC :show="isOpen" @close="close" title="Crear Tarea" @crear="crear"></ModalC>
</template>
