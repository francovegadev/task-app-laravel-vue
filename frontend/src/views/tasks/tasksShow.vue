...
<template>
  <section class="bg-neutral-primary" v-if="taskStore.task && taskStore.task.image?.path">
    <div class="py-8 px-4 mx-auto max-w-screen-2xl text-center lg:py-16">
      <h1
        class="mb-6 text-4xl font-bold tracking-tighter text-heading md:text-5xl flex flex-col lg:text-6xl"
      >
        Detalle de la tarea: <span class="text-faded">{{ taskStore.task.title }}</span>
      </h1>
      <div
        class="flex flex-col bg-navbarbg px-4 py-4 rounded-sm shadow-sm w-full max-w-2xl mx-auto"
      >
        <div class="flex w-full border border-faded justify-center rounded-sm">
          <figure class="max-w-lg">
            <img
              class="h-48 max-w-full rounded-sm"
              :src="taskStore.task.image.path"
      ...
      <div
        class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0 md:space-x-4 my-4"
      >
        <router-link
          :to="{ name: 'tasksView' }"
